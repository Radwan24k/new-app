...
router.get("/favorites", auth, async (req, res) => {
  ...
  const favoriteCards = await Card.find({ _id: { $in: user.cards } });
...
router.patch("/:id/favorite", auth, async (req, res) => {
  ...
  const isFavorite = user.cards.includes(cardId);
  ...
  await user.save();
  res.send(user.cards);
  console.log("Toggling favorite for card ID:", req.params.id);
});
